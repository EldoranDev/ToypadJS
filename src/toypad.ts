...
export class Toypad {
  ...
  public async init(): Promise<void> {
    ...
    this.device.addListener((data: Uint8Array) => {
      console.debug(
        "RX: ",
        [...data].map((x) => x.toString(16).padStart(2, "0")).join(" "),
      );
      ...
      if (frame.type === FrameType.Message) {
        if (this.promiseMap.has(frame.messageId.toString())) {
          this.promiseMap.get(frame.messageId.toString())!(frame);
  ...
  public wake(): Promise<void> {
    return new Promise((resolve) => {
      this.send(
        ...
        [
          // Hex of: (c) LEGO 2014
          0x28, 0x63, 0x29, 0x20, 0x4c, 0x45, 0x47, 0x4f, 0x20, 0x32, 0x30,
          0x31, 0x34,
        ],
        () => {
          resolve();
  ...
  public async setColor(pad: Pad, color: Color): Promise<void> {
    await this.send(Command.Color, [pad, ...color]);
  }
  ...
  public async setColorAll(
    center: Color,
    left: Color,
    right: Color,
  ): Promise<void> {
    await this.send(Command.ColorAll, [1, ...center, 1, ...left, 1, ...right]);
  }
  ...
  public async fadeRandom(
    pad: Pad,
    tickTime: number,
    tickCount: number,
  ): Promise<void> {
    await this.send(Command.FadeRandom, [pad, tickTime, tickCount]);
  }
      ...
      this.send(Command.TagList, [], (frame) => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const tag = {
            ...
            pad: high,
            status:
              frame.data[i * 2 + 1] === 0 ? TagStatus.Ok : TagStatus.Error,
  ...
  public readTag(index: number, page: number): Promise<Uint8Array> {
    return new Promise((resolve, reject) => {
      this.send(Command.ReadTag, [index, page], (frame: Frame) => {
  ...
  public writeTag(
    ...
    return new Promise((resolve) => {
      this.send(Command.WriteTag, [index, page, ...data], (frame: Frame) => {
  ...
  private async send(
    ...
    if (undefined !== callback) {
      this.promiseMap.set(messageId.toString(), callback);
    }
  ...
  static async connect(
    ...
    return new Toypad(await getDevice(), addCallback, removeCallback);
  }
}
